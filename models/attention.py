...
class SelfAttention_residual(nn.Module):
    ...
    def forward(self, x):
        ...
        g_x = self.g(x).view(batch_size, self.inter_channels, -1) # (b, d, N)
        g_x = g_x.permute(0, 2, 1) # (b, N, d)
        ...
        if self.mode == "gaussian":
            ...
            theta_x = self.theta(x).view(batch_size, self.inter_channels, -1) # (b, d, N)
            phi_x = self.phi(x).view(batch_size, self.inter_channels, -1) # (b, d, N)
            theta_x = theta_x.permute(0, 2, 1) # (b, N, d)
        ...
        if self.mode == "gaussian" or self.mode == "embedded":
            f_div_C = F.softmax(f, dim=-1) # softmax score
        elif self.mode == "dot" or self.mode == "concatenate":
            N = f.size(-1)  # number of position in x
            f_div_C = f / N

        y = torch.matmul(f_div_C, g_x)  # (b, N, d)

        # contiguous here just allocates contiguous chunk of memory
        y = y.permute(0, 2, 1).contiguous()
        y = y.view(batch_size, self.inter_channels, *x.size()[2:]) # (b, d, N)
