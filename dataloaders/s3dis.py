...
class S3DISDataset(object):
    def __init__(self, cvfold, data_path):
        '''
        goal: generate class2scan from clean label!! data[:,6]
        data format: xyzrgb + clean label
        :param cvfold: decide self.test_classes
        :param data_path:
        '''
        ...
        # class_names = open(os.path.join('/home/yating/Documents/3d_segmentation/attMPTI-noise/datasets/S3DIS', 'meta', 's3dis_classnames.txt')).readlines()
        ...
        self.class2scans = self.get_class2scans() # for all classes (13 classes!)
    ...
    def get_class2scans(self):
        ...
        if os.path.exists(class2scans_file):
            ...
            for file in glob.glob(os.path.join(self.data_path, 'data', '*.npy')):
                ...
                data = np.load(file)
                labels = data[:,6].astype(np.int) # clean label
