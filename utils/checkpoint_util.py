...
def load_pretrain_checkpoint(model, pretrain_checkpoint_path):
    ...
    if pretrain_checkpoint_path is not None:
        ...
        pretrained_dict = torch.load(pretrain_checkpoint_path)['params']
...
def save_pretrain_checkpoint(model, output_path, epoch=None):
    if epoch != None:
        torch.save(dict(params=model.encoder.state_dict()), os.path.join(output_path, 'checkpoint_{}.tar'.format(epoch)))
    else:
